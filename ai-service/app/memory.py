...
key 结构：ai:chat:history:{userId}:{petId}（LIST，每项为 JSON 字符串 {"role","content","ts"}）
...
def _key(user_id: int, pet_id: int) -> str:
    """拼接对话历史的 Redis Key：ai:chat:history:{userId}:{petId}"""
    return f"ai:chat:history:{user_id}:{pet_id}"
...
async def read(user_id: int, pet_id: int) -> List[dict]:
    ...
    if _client is None:
        return []
    try:
        raw_list = await _client.lrange(_key(user_id, pet_id), 0, -1)
        result: List[dict] = []
        for raw in raw_list:
            try:
                item = json.loads(raw)
...
async def append(user_id: int, pet_id: int, user_msg: str, reply: str) -> None:
    """追加一轮对话（用户消息 + 助手回复）
...
# 避免「先 exists 检查、再写入」两步之间 key 被 DELETE（用户清空对话）导致已删的历史复活。
...
async def append_if_exists(user_id: int, pet_id: int, user_msg: str, reply: str) -> None:
    """条件追加一轮对话：仅当历史 key 仍存在时才写入，key 不存在则跳过

    语义区分：正常对话结束用 append（允许创建新 key）；
    客户端断开 / 停止生成的兜底保存用本方法——若用户此刻已清空对话（key 被 DELETE），
    直接跳过写入，避免把刚删掉的历史"复活"。
    任何异常仅记日志，不影响对话主流程。
    """
    if _client is None:
...
async def clear(user_id: int, pet_id: int) -> None:
    """清空指定宠物的对话记忆；任何异常仅记日志"""
    if _client is None:
        return
    try:
        await _client.delete(_key(user_id, pet_id))
    except Exception:
